...
contract HigherrrrrrrConvictionTest is Test {
    ...
    function substringWithLength(string memory str, uint256 startIndex, uint256 length)
        internal
        pure
    ...
    function _containsString(string memory _string, string memory search) private pure returns (bool) {
    ...
    function formatEthValue(uint256 weiAmount) internal pure returns (string memory) {
        ...
        return string(abi.encodePacked(wholeNumber.toString(), ".", decimals.toString()));
    }
    ...
    function test_NFTDisplayAmounts() public {
        ...
        assertTrue(_containsString(uri, wholeNumber), "URI should contain whole number part");
    ...
    function test_NFTAmountSanitization() public {
        ...
        uint256 tokenId = conviction.mintConviction(user1, "highr<script>alert('xss')</script>", 1000e18, price);
        ...
        assertTrue(!_containsString(uri, "<script>"), "Raw script tags should not be present");
    ...
    function test_URIFormat() public {
        ...
        assertTrue(_containsString(decodedJson, "\"image\""), "URI should contain image field");

        assertTrue(_containsString(decodedJson, "\"attributes\""), "URI should contain attributes field");
    ...
    function test_GetHigherrrrrrrState() public {
        ...
        vm.mockCall(token, abi.encodeWithSignature("name()"), abi.encode("highrrrrrr"));

        vm.mockCall(token, abi.encodeWithSignature("getCurrentPrice()"), abi.encode(1 ether));

        vm.mockCall(
            token,
            abi.encodeWithSignature("state()"),
            abi.encode(
                IHigherrrrrrr.MarketState({
    ...
    function testFail_TokenURIInputTooLong() public {
        ...
        uint256 tokenId = conviction.mintConviction(user1, longString, 1000e18, 1 ether);
    ...
    function test_TokenURIEdgeCases() public {
        ...
        assertTrue(
            _containsString(decodedJson1, "0.000000000000000001") || _containsString(decodedJson1, "1 wei")
                || _containsString(decodedJson1, "0.1e-17") || _containsString(decodedJson1, "0.1"),
        ...
        uint256 tokenId3 = conviction.mintConviction(user1, unicode"highr©", 1000e18, 1 ether);
    ...
    function test_ComplexSanitization() public {
        ...
        string memory complexHtml =
            '<div onclick="alert(1)" style="color: red"><script>evil()</script><img src="x" onerror="alert(1)"/></div>';
        ...
        string memory controlChars = string(abi.encodePacked("bad", bytes1(0x00), "chars", bytes1(0x0A), "here"));
    ...
    function test_SVGRendering() public {
        ...
        // SVG Checks
        assertTrue(_containsString(svgContent, "font-family: monospace"), "Should have monospace font family");

        assertTrue(_containsString(svgContent, "#4afa4a"), "Should have correct color");

        assertTrue(
            _containsString(svgContent, "<svg") && _containsString(svgContent, "viewBox"),
            "Should have SVG root element with viewBox"
        );

        assertTrue(
            _containsString(svgContent, "tokens") && _containsString(svgContent, "ETH"),
    ...
    function _indexOf(string memory _str, string memory _sub) internal pure returns (int256) {
        ...
        for (uint256 i = 0; i <= str.length - sub.length; i++) {
            bool found = true;
    ...
    function test_TokenURIPriceFormatting() public {
        ...
        tests[0] = PriceTest({price: 1.5 ether, expectedFormat: "1.5"});
        tests[1] = PriceTest({price: 1 ether, expectedFormat: "1.0"});
        tests[2] = PriceTest({price: 0.1 ether, expectedFormat: "0.1"});
        tests[3] = PriceTest({price: 1000 ether, expectedFormat: "1000.0"});
